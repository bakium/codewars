...
describe('cycle', function () {
    it('gets the next value to the left', function () {
        assert.strictEqual(cycle(-1, [1, 2, 3], 2), 1);
    });
    it('gets the next value to the left', function () {
        assert.strictEqual(cycle(-4, [1, 2, 3], 2), 1);
    });
    it('gets the next value to the left', function () {
        assert.strictEqual(cycle(-19, [1, 2, 3], 2), 1);
    });
    it('gets the next value to the right', function () {
        assert.strictEqual(cycle(1, [1, 2, 3], 1), 2);
    });
    it('gets the next value to the right', function () {
        assert.strictEqual(cycle(1, [1, 2, 3], 3), 1);
    });
    it('gets the next value to the right', function () {
        assert.strictEqual(cycle(1, [1, 2, 2, 3], 2), 2);
    });
});
